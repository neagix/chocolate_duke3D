                         ...
                         EngineState *engine_state)
{
    ...
    for (; (x<=x2)&&((x+frameoffset-(uint8_t *)NULL)&3); x++) {
        ...
        mvlineasm1(vince[0],palookupoffse[0],y2ve[0]-y1ve[0]-1,vplce[0],bufplce[0]+tiles[globalpicnum].data,x+frameoffset+ylookup[y1ve[0]]);
    }
    ...
    for (; x<=x2-3; x+=4) {
        ...
        if ((bad != 0) || (u4 >= d4)) {
            for (int j=0; j<4; j++) {
                if (!(bad & (1<<j))) {
                    mvlineasm1(vince[j], palookupoffse[j], y2ve[j]-y1ve[j], vplce[j], bufplce[j], ylookup[y1ve[j]]+x+frameoffset+j);
                }
            }
            continue;
        }
        ...
        for (int j=0; j<4; j++) {
            if (u4 > y1ve[j]) {
                vplce[j] = mvlineasm1(vince[j], palookupoffse[j], u4-y1ve[j]-1, vplce[j], bufplce[j], ylookup[y1ve[j]] + x + frameoffset + j);
            }
        }
        ...
        for (int j=0; j<4; j++) {
            if (y2ve[j] > d4) {
                mvlineasm1(vince[j], palookupoffse[j], y2ve[j]-d4-1, vplce[j], bufplce[j], i+j);
            }
        }
    }
...
static void drawalls(int32_t bunch, short *numscans, short *numhits, short *numbunches, EngineState *engine_state)
{
    ...
    for (z=bunchfirst[bunch]; z>=0; z=bunchWallsList[z]) {
        ...
        if (nextsectnum >= 0) {
            ...
            if (((sec->ceiling.stat&1) == 0) || ((nextsec->ceiling.stat&1) == 0)) {
                if ((cz[2] <= cz[0]) && (cz[3] <= cz[1])) {
                    ...
                    wallscan(x1, x2, uplc, dwall, swall, lwall, zd, xpanning, engine_state);
            ...
            if (((sec->floor.stat&1) == 0) || ((nextsec->floor.stat&1) == 0)) {
                if ((fz[2] >= fz[0]) && (fz[3] >= fz[1])) {
                    ...
                    wallscan(x1, x2, uwall, dplc, swall, lwall, zd, xpanning, engine_state);
        ...
        if ((nextsectnum < 0) || (wal->cstat&32)) { /* White/1-way wall */
            ...
            wallscan(x1, x2, uplc, dplc, swall, lwall, zd, xpanning, engine_state);
...
static void transmaskvline(int32_t x, int32_t zd, int32_t xpanning, EngineState *engine_state)
{
    int32_t vplc, vinc, i;
    short y1v, y2v;
...
static void drawsprite (EngineState *engine_state, int32_t *spritesx, int32_t *spritesy)
{
    ...
    if ((cstat&48) == 0) {
        ...
        if ((cstat&2) == 0) {
            maskwallscan(lx,rx,uwall,dwall,swall,lwall, zd, xpanning, engine_state);
        } else {
            transmaskwallscan(lx, rx, zd, xpanning, engine_state);
        }
    } else if ((cstat&48) == 16) {
        if ((cstat&4) > 0) {
            xoff = -xoff;
