...
int32_t ifsquished(short i, short p)
{
    ...
    sc = &sector[SECT];
    floorceildist = sc->floor.z - sc->ceiling.z;
...
void hitradius( short i, int32_t  r, int32_t  hp1, int32_t  hp2, int32_t  hp3, int32_t  hp4 )
{
    ...
    if (s->picnum != SHRINKSPARK) {
        ...
        do {
            ...
            if (((sector[dasect].ceiling.z-s->z)>>8) < r) {
...
short ssp(short i,uint32_t cliptype) //The set sprite function
{
    ...
    movetype = movesprite(i,
                          (s->xvel * (fixedPointCos(s->ang))) >> 14,
                          (s->xvel * (fixedPointSin(s->ang))) >> 14,
                          s->zvel,
                          cliptype);
...
void ms(short i)
{
    ...
    s->x += (s->xvel * (fixedPointCos(s->ang))) >> 14;
    s->y += (s->xvel * (fixedPointSin(s->ang))) >> 14;
        ...
        if (sprite[sn].extra >= 0 ) {
            ...
            if (npc->picnum == APLAYER) {
                ...
                switch (hittype[sn].picnum) {
                    ...
                    case EXPLODINGBARREL:
                        ps[p].posxv +=
                            hittype[sn].extra*(fixedPointCos(hittype[sn].ang))<<2;
                        ps[p].posyv +=
                            hittype[sn].extra*(fixedPointSin(hittype[sn].ang))<<2;
                        break;
                    default:
                        ps[p].posxv +=
                            hittype[sn].extra*(fixedPointCos(hittype[sn].ang))<<1;
                        ps[p].posyv +=
                            hittype[sn].extra*(fixedPointSin(hittype[sn].ang))<<1;
                        break;
                }
            } else {
                if (hittype[sn].extra == 0 )
...
//                    sprite[j].pal = sprite[i].pal;
            ...
            case MUSICANDSFX:
                ...
                } else if (s->lotag < 999 && (uint32_t)sector[s->sectnum].lotag < 9 && AmbienceToggle && sector[SECT].floor.z != sector[SECT].ceiling.z) {
                    if ( (soundm[s->lotag]&2) ) {
    ...
    while (i >= 0) {
        ...
        if ( T1 == 0 ) {
            ...
            if (s->lotag > 0) {
                ...
                if ( s->z < (sector[sect].floor.z-FOURSLEIGHT) ) {
                    s->zvel += x;
                    if (s->zvel > 6144) {
                        s->zvel = 6144;
                    }
                ...
                if ( (sector[sect].floor.z-s->z) < (16<<8) ) {
                    j = 1+(TRAND&7);
                    for (x=0; x<j; x++) {
                        RANDOMSCRAP;
                    }
    ...
    while (i >= 0) {
        ...
        IFWITHIN(CRANE,CRANE+3) {
            ...
            if ( t[0] == 0 ) { //Waiting to check the sector
                ...
                if (t[0]==2) {
                    if ( (sector[sect].floor.z - s->z) < (64<<8) )
                        if (s->picnum > CRANE) {
                            s->picnum--;
                        }

                    if ( (sector[sect].floor.z - s->z) < (4096+1024)) {
                        t[0]++;
                    }
                }
                if (t[0]==7) {
                ...
                if (t[0]==8 && s->picnum < (CRANE+2))
                    if ( (sector[sect].floor.z-s->z) > 8192) {
                        s->picnum++;
                    }
            ...
            if (s->owner != -1) {
                ...
                if (s->owner >= 0) {
                    ...
                    ps[p].oposx = ps[p].posx = s->x-(fixedPointCos(ps[p].ang)>>6);
                    ps[p].oposy = ps[p].posy = s->y-(fixedPointSin(ps[p].ang)>>6);
        ...
        if (s->picnum == TRIPBOMB) {
            ...
            if ( T1 == 32 ) {
                ...
                T4 = s->x;
                T5 = s->y;
                s->x += fixedPointCos(T6) >> 9;
                s->y += fixedPointSin(T6) >> 9;
                s->z -= (3<<8);
                setsprite(i,s->x,s->y,s->z);
                ...
                while (x > 0) {
                    ...
                    s->x += fixedPointCos(T6)>>4;
                    s->y += fixedPointSin(T6)>>4;
...
CLEAR_THE_BOLT:
            ...
            case DOORSHOCK:
                j = klabs(sector[sect].ceiling.z-sector[sect].floor.z)>>9;
                s->yrepeat = j+4;
                s->xrepeat = 16;
                s->z = sector[sect].floor.z;
                goto BOLT;

            case TOUCHPLATE:
                if ( t[1] == 1 && s->hitag >= 0) { //Move the sector floor
                    x = sector[sect].floor.z;

                    if (t[3] == 1) {
                        if (x >= t[2]) {
                            sector[sect].floor.z = x;
                            t[1] = 0;
                        } else {
                            sector[sect].floor.z += sector[sect].extra;
...
void bounce(short i)
{
    ...
    day = mulscale14(k,fixedPointSin((daang+1536)));
    daz = 4096;
    ...
    while (i >= 0) {
        ...
        switch (s->picnum) {
            ...
            case COOLEXPLOSION1:
                ...
                j = movesprite(i,
                               (k*(fixedPointCos(s->ang)))>>14,
                               (k*(fixedPointSin(s->ang)))>>14,ll,qq);
            ...
            switch (sprite[j].statnum) {
                case 10:    // Player
                    ...
                    if ( sprite[j].owner != -1 ) {
                        ...
                        if (onfloorz && sectlotag == 1 &&
                            ps[p].on_ground &&
                            ps[p].posz > (sector[sect].floor.z-(16<<8)) &&
                            ((sync[p].bits&2) || ps[p].poszv > 2048))
//                        if( onfloorz && sectlotag == 1 && ps[p].posz > (sector[sect].floor.z-(6<<8)) )
                        {
                            k = 1;
                            if (screenpeek == p) {
                                FX_StopAllSounds();
                            ...
                            ps[p].oposz = ps[p].posz =
                                              sector[sprite[OW].sectnum].ceiling.z + (7<<8);
                        ...
                        if ( onfloorz && sectlotag == 2 && ps[p].posz < (sector[sect].ceiling.z+(6<<8)) ) {
                            k = 1;
//                            if( sprite[j].extra <= 0) break;
                            ...
                            ps[p].oposz = ps[p].posz =
                                              sector[sprite[OW].sectnum].floor.z-(7<<8);
                ...
                case 13:
                    ...
                    {
                        warpspriteto = 0;
                        if ( ll && sectlotag == 2 && sprite[j].z < (sector[sect].ceiling.z + ll) ) {
                            warpspriteto = 1;
                        }

                        if ( ll && sectlotag == 1 && sprite[j].z > (sector[sect].floor.z-ll) ) {
                            warpspriteto = 1;
                        }
...
//                                if( rnd(192) && sprite[j].picnum == WATERBUBBLE)
                                    ...
                                    switch (sectlotag) {
                                        case 0:
                                            if (onfloorz) {
                                                if ( sprite[j].statnum == 4 || ( checkcursectnums(sect) == -1 && checkcursectnums(sprite[OW].sectnum)  == -1 ) ) {
                                                    ...
                                                    sprite[j].z -= SZ - sector[sprite[OW].sectnum].floor.z;
                                        ...
                                        case 1:
                                            sprite[j].x += (sprite[OW].x-SX);
                                            sprite[j].y += (sprite[OW].y-SY);
                                            sprite[j].z = sector[sprite[OW].sectnum].ceiling.z + ll;
                                        ...
                                        case 2:
                                            sprite[j].x += (sprite[OW].x-SX);
                                            sprite[j].y += (sprite[OW].y-SY);
                                            sprite[j].z = sector[sprite[OW].sectnum].floor.z - ll;
        ...
        switch (s->picnum) {
            ...
            case RAT:
                ...
                IFMOVING {
                    ...
                    s->ang += (TRAND&31)-15+(fixedPointSin((t[0]<<8))>>11);
                }
                else {
                    T1++;
                    if (T1 > 1) {
            ...
            case STRIPEBALL:
                if (s->xvel) {
                    ...
                    j = clipmove(&s->x,&s->y,&s->z,&s->sectnum,
                                 (((s->xvel*(fixedPointCos(s->ang)))>>14)*TICSPERFRAME)<<11,
                                 (((s->xvel*(fixedPointSin(s->ang)))>>14)*TICSPERFRAME)<<11,
...
//                        if(s->pal == 12)
                    ...
                    for (l=512; l<(2048-512); l+= 128)
                        for (j=0; j<2048; j += 128) {
                            ...
                            *(int16_t *)&sprite[k].flags = 257+128;
                            ...
                            sprite[k].xvel = fixedPointCos(l)>>9;
                            sprite[k].owner = i;
                        }
                }
            ...
            case RECON:
                ...
                if ( s->z < sector[sect].ceiling.z+(32<<8) ) {
                    s->z = sector[sect].ceiling.z+(32<<8);
                }
...
// #endif
                ...
                if (t[0] == -4) { //On the player
                    ...
                    s->xrepeat = 20+(fixedPointSin(t[1])>>13);
                    s->yrepeat = 15+(fixedPointSin(t[1])>>13);

                    s->x = ps[p].posx + (fixedPointSin(ps[p].ang)>>7);
                    s->y = ps[p].posy + (fixedPointSin(ps[p].ang)>>7);
...
//                    if(s->yrepeat > 62)
                ...
                if (t[0] == -2) { //On top of somebody
                    ...
                    s->z = sprite[t[5]].z;
                    s->x = sprite[t[5]].x+(fixedPointCos(l)>>11);
                    s->y = sprite[t[5]].y+(fixedPointSin(l)>>11);
            ...
            case HEAVYHBOMB:
                ...
                if ( s->picnum != BOUNCEMINE ) {
                    ...
                    if ( sector[sect].lotag != 1 && s->z >= hittype[i].floorz-(FOURSLEIGHT) && s->yvel < 3 ) {
                        if ( s->yvel > 0 || (s->yvel == 0 && hittype[i].floorz == sector[sect].floor.z )) {
...
DETONATEB:
            ...
            case REACTOR2:
                ...
                if ( t[1] ) {
                    ...
                    t[4] = s->z;
                    s->z = sector[sect].floor.z-(TRAND%(sector[sect].floor.z - sector[sect].ceiling.z));
    ...
    while (i >= 0) {
        ...
        switch (s->picnum) {
            ...
            case PAPER:
                ...
                s->xvel = (TRAND&7)+(fixedPointSin(T1)>>9);
                T1 += (TRAND&63);
            ...
            case SHOTGUNSHELL:
                ...
                if (sect < 0 || ( sector[sect].floor.z+(24<<8) ) < s->z ) {
                    KILLIT(i);
                }
        ...
        IFWITHIN(SCRAP6,SCRAP5+3) {
            ...
            if ( s->z < sector[sect].floor.z-(2<<8) ) {
                if (t[1] < 1) {
                    t[1]++;
                } else {
                    t[1] = 0;
            ...
            case 0: {
                ...
                if (sc->lotag == 30) {
                    ...
                    if ( sprite[i].extra == 1 ) {
                        ...
                        if ( sc->floor.z > s->z ) { //z's are touching
                            sc->floor.z -= 512;
                            zchange = -512;
                            if ( sc->floor.z < s->z ) {
                                sc->floor.z = s->z;
                        ...
                        else if ( sc->floor.z < s->z ) { //z's are touching
                            sc->floor.z += 512;
                            zchange = 512;
                            if ( sc->floor.z > s->z ) {
                                sc->floor.z = s->z;
                        ...
                        if ( sc->floor.z > T4 ) { //z's are touching
                            sc->floor.z -= 512;
                            zchange = -512;
                            if ( sc->floor.z < T4 ) {
                                sc->floor.z = T4;
                        ...
                        else if ( sc->floor.z < T4 ) { //z's are touching
                            sc->floor.z += 512;
                            zchange = 512;
                            if ( sc->floor.z > T4 ) {
                                sc->floor.z = T4;
...
//                        if( t[4] == 1 )
            ...
            case 14:
                ...
                if (s->xvel) {
                    ...
                    if ( (sc->floor.z - sc->ceiling.z) < (108<<8) ) {
                    ...
                    m = (s->xvel*fixedPointCos(s->ang))>>14;
                    x = (s->xvel*fixedPointSin(s->ang))>>14;
            ...
            case 30:
                ...
                if (s->xvel) {
                    l = (s->xvel*fixedPointCos(s->ang))>>14;
                    x = (s->xvel*fixedPointSin(s->ang))>>14;
            ...
            case 9:
                ...
                if (hittype[i].temp_data[4]) {
                    ...
                    j = getanimationgoal(&sc->ceiling.z);
                }
            ...
            case 10:
                ...
                if ((sc->lotag&0xff) == 27 ||
                    (sc->floor.z > sc->ceiling.z && (sc->lotag&0xff) != 23 ) ||
                    ...
                    if (j == 1) {
                        if (t[0] > sh )
                            switch (sector[s->sectnum].lotag) {
                                ...
                                case 22:
                                case 26:
                                    if ( getanimationgoal(&sector[s->sectnum].ceiling.z) >= 0 ) {
                                        break;
                                    }
                                default:
            ...
            case 13:
                if ( t[2] ) {
                    ...
                    if ( s->ang == 512 ) {
                        if ( s->owner ) {
                            if ( klabs(t[0]-sc->ceiling.z) >= j ) {
                                sc->ceiling.z += sgn(t[0]-sc->ceiling.z)*j;
                            } else {
                                sc->ceiling.z = t[0];
                            }
                        } else {
                            if ( klabs(t[1]-sc->floor.z) >= j ) {
                                sc->floor.z += sgn(t[1]-sc->floor.z)*j;
                            } else {
                                sc->floor.z = t[1];
                            }
                        }
            ...
            case 16: //Reactor
                ...
                t[2]+=32;
                if (sc->floor.z<sc->ceiling.z) {
                    s->shade=0;
                }

                else if ( sc->ceiling.z < t[3] ) {
            ...
            case 17:
                ...
                while (j >= 0) {
                    ...
                    hittype[j].floorz = sc->floor.z;
                    hittype[j].ceilingz = sc->ceiling.z;

                    j = nextspritesect[j];
                }

                if ( t[0] )                if (t[0]) { //If in motion
                        if ( klabs(sc->floor.z-t[2]) <= SP) {
                        ...
                        while (k >= 0) {
                            ...
                            if (sprite[k].statnum == 10 && sprite[k].owner >= 0) {
                                ...
                                ps[p].posz = sector[sprite[j].sectnum].floor.z-(sc->floor.z-ps[p].posz);

                                hittype[k].floorz = sector[sprite[j].sectnum].floor.z;
                                hittype[k].ceilingz = sector[sprite[j].sectnum].ceiling.z;
                                ...
                                sprite[k].y +=
                                    sprite[j].y-s->y;
                                sprite[k].z = sector[sprite[j].sectnum].floor.z-
                                              (sc->floor.z-sprite[k].z);
            ...
            case 19: //Battlestar galactia shields
                ...
                if (t[0]) {
                    ...
                    if (sc->ceiling.z < sc->floor.z) {
                        sc->ceiling.z += SP;
                    } else {
                        sc->ceiling.z = sc->floor.z;
...
//                                case 18:
            ...
            case 20: //Extend-o-bridge
                ...
                if ( s->xvel ) { //Moving
                    x = (s->xvel*fixedPointCos(s->ang))>>14;
                    l = (s->xvel*fixedPointSin(s->ang))>>14;
            ...
            case 21: // Cascading effect
                ...
                if ( s->ang == 1536 ) {
                    l = (int32_t) &sc->ceiling.z;
                } else {
                    l = (int32_t) &sc->floor.z;
                }
            ...
            case 22:
                ...
                if ( t[1] ) {
                    if (getanimationgoal(&sector[t[0]].ceiling.z) >= 0) {
                        sc->ceiling.z += sc->extra*9;
                    } else {
                        t[1] = 0;
                    }
                }
            ...
            case 34:
                ...
                x = (SP*fixedPointCos(s->ang))>>18;
                l = (SP*fixedPointSin(s->ang))>>18;
            ...
            case 35:
                ...
                switch (t[0]) {
                    case 0:
                        sc->ceiling.z += s->yvel;
                        if (sc->ceiling.z > sc->floor.z) {
                            sc->floor.z = sc->ceiling.z;
                        }
                        if (sc->ceiling.z > s->z+(32<<8)) {
                            t[0]++;
                        }
                        break;
                    case 1:
                        sc->ceiling.z-=(s->yvel<<2);
                        if (sc->ceiling.z < t[4]) {
                            sc->ceiling.z = t[4];
                            t[0] = 0;
                        }
            ...
            case 26:
                ...
                if ( s->shade > 7 ) {
                    s->x = t[3];
                    s->y = t[4];
                    sc->floor.z -= ((s->zvel*s->shade)-s->zvel);
                    s->shade = 0;
                } else {
                    sc->floor.z += s->zvel;
                }
            ...
            case 29:
                s->hitag += 64;
                l = mulscale12((int32_t)s->yvel,fixedPointSin(s->hitag));
                sc->floor.z = s->z + l;
                break;
            ...
            case 32: // True Drop Ceiling
                if (t[0] == 1) {
                        ...
                        if (SA != 1536) {
                            if ( klabs( sc->ceiling.z - s->z ) <
                                ...
                                t[0] = 0;
                                    ...
                                    sgn(s->z-sc->ceiling.z)*SP;
                        } else {
                            if ( klabs( sc->ceiling.z - t[1] ) <
                                 (SP<<1) ) {
                                sc->ceiling.z = t[1];
                                callsound(s->sectnum,i);
                                t[2] = 0;
                    ...
                    if ( (s->ang&2047) == 1536) {
                        ...
                        if ( klabs(sc->ceiling.z-t[1] ) < (SP<<1) ) {
                            t[0] = 0;
                            t[2] = !t[2];
                            callsound(s->sectnum,i);
                        } else {
                            sc->ceiling.z -= sgn(s->z-t[1])*SP;
                        }
                    }
                }
                break;
            ...
            case 130:
                ...
                x = sc->floor.z-sc->ceiling.z;

                if ( rnd(64) ) {
                    k = spawn(i,EXPLOSION2);
                    sprite[k].xrepeat = sprite[k].yrepeat = 2+(TRAND&7);
                    sprite[k].z = sc->floor.z-(TRAND%x);
...
BOLT:
    ...
    for (i=headspritestat[3]; i>=0; i=nextspritestat[i]) {
        ...
        alignflorslope(s->sectnum,wal->x,wal->y,sector[wal->nextsector].floor.z);
    }
}
