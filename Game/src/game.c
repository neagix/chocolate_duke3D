                ...
                if ( ( display_mirror == 1 || screenpeek != p || s->owner == -1 ) && ud.multimode > 1 && ud.showweapons && sprite[ps[p].i].extra > 0 && ps[p].curr_weapon > 0 ) {
                    ...
                    *(int16_t *)&tsprite[engine_state->spritesortcnt].flags = 0;
                    ...
                    switch (ps[p].curr_weapon) {
                        case PISTOL_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = FIRSTGUNSPRITE;
                            break;
                        case SHOTGUN_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = SHOTGUNSPRITE;
                            break;
                        case CHAINGUN_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = CHAINGUNSPRITE;
                            break;
                        case RPG_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = RPGSPRITE;
                            break;
                        case HANDREMOTE_WEAPON:
                        case HANDBOMB_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = HEAVYHBOMB;
                            break;
                        case TRIPBOMB_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = TRIPBOMBSPRITE;
                            break;
                        case GROW_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = GROWSPRITEICON;
                            break;
                        case SHRINKER_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = SHRINKERSPRITE;
                            break;
                        case FREEZE_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = FREEZESPRITE;
                            break;
                        case DEVISTATOR_WEAPON:
                            tsprite[engine_state->spritesortcnt].picnum = DEVISTATORSPRITE;
                            break;
                    }
                    ...
                    if (s->owner >= 0) {
                        ...
                        tsprite[engine_state->spritesortcnt].z = s->z-(51<<8);
                    }
...
uint8_t  opendemoread(uint8_t  which_demo) // 0 = mine
{
    ...
    // FIX_00034: Demos do not turn your run mode off anymore:
    /* ud.showweapons =  */
    ud.pause_on /*= ud.auto_run */ = 0; // makes no sense to reset those 2 value!
